...
async function getOwnedTokenIds(contract, wallet) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const tokenIds = await getOwnedTokenIds(contract, wallet);
    const traitCount = {};

    for (const tokenId of tokenIds) {
      try {
        const uri = await contract.tokenURI(tokenId);
        const meta = await fetchWithFallback(ipfsToHttp(uri));
